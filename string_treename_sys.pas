...
var
  ...
  len: sys_int_machine_t;              {length of current pathname component}
  ...
  comp: string_leafname_t;             {scratch pathname component}
  stat: sys_err_t;
...
label
  next_comp, retry_comp, got_comp, cmd_end, done_special, abort;

begin
  path.max := size_char(path.str);     {init local var strings}
  cmd.max := size_char(cmd.str);
  comp.max := size_char(comp.str);
...
retry_comp:                            {try again to find a component}
  ...
  while p <= iname_len do begin        {scan remaining chars in INAME}
    case iname.str[p] of               {what kind of character is this ?}
'/', '\': begin                        {separator between components}
        len := p - st;                 {make length of this component}
        p := p + 1;                    {make index of first char in next component}
        if len > 0 then goto got_comp; {we have a complete pathname component}
...
':': begin                             {just hit end of drive name}
        string_substr (iname, st, p, t); {replace path with drive name}
        string_upcase (t);             {drive names are always upper case}
        p := p + 1;                    {skip over ":"}
        no_comp := false;              {at least one component has been processed}
        goto next_comp;
        end;
  ...
  len := p - st;                       {make length of this last component}
{
*   The next pathname component has been identified.  It is in INAME,
*   starts at index ST, and is LEN characters long.
}
got_comp:
  no_comp := false;                    {at least one component has been found now}

  case iname.str[st] of                {check for special case component names}
'.': begin                             {could be current or parent directory}
      if len = 1 then goto retry_comp; {current directory ?}
      if                               {parent directory ?}
          (len = 2) and then
          (iname.str[st+1] = '.')
          then begin
...
'~': begin                             {could be user's home directory}
      if len <> 1 then goto done_special; {not just "~" symbol ?}
...
'(': begin                             {embedded command}
      cmd.len := 0;                    {init command name string to empty}
      for i := st+1 to st+len-1 do begin {scan remaining characters this component}
        if iname.str[i] = ')'          {found end of command name character ?}
          then goto cmd_end;
        if cmd.len < cmd.max then begin {room for one more characters ?}
          cmd.len := cmd.len + 1;      {append this char to end of command name}
          cmd.str[cmd.len] := iname.str[i];
          end;
        end;                           {back for next cmd char in INAME}
      goto done_special;               {")" missing, not a command ?}
cmd_end:                               {I is INAME index of command end ")"}
      comp.len := min(comp.max, len - (i - st + 1)); {make length after command}
      for j := 1 to comp.len do begin  {copy component remainder into COMP}
        comp.str[j] := iname.str[i + j];
        end;
      ...
      case pick of
1: begin                               {COG, Cognivision installation subdirectory}
          string_terminate_null (comp); {make sure COMP.STR is null terminated}
          sys_cognivis_dir (comp.str, path); {get Cognivis dir path}
          do_pathname (path, opts);    {add on expanded Cognivis directory path}
          if tstat <> string_tnstat_native_k then goto abort; {special condition ?}
          end;
2: begin                               {EVAR, environment variable expansion}
          if comp.len <= 0 then goto next_comp; {ignore if no environment var name given}
          if not (string_tnamopt_proc_k in opts) then begin {we don't own pathname ?}
            tstat := string_tnstat_proc_k; {need to be translated by owning process}
            goto abort;                {return with special condition}
            end;
          sys_envvar_get (comp, path, stat); {get environment variable value}
          if sys_error(stat) then begin {couldn't get envvar value ?}
            sys_error_none (stat);     {reset error condition}
            goto next_comp;            {ignore if envvar doesn't exist}
            end;
          do_pathname (path, opts);    {add on environment variable expansion}
          if tstat <> string_tnstat_native_k then goto abort; {special condition ?}
          end;
otherwise                              {unrecognized command}
      ...
      end;                             {end of embedded command case}
    end;                               {done handling special case component names}
done_special:                          {definately done with special handling}
{
*   This input pathname component is not one of the special case syntaxes.
...
comptype_netroot_k: begin              {component is at network root (machine name)}
      string_substr (iname, st, st + len - 1, path); {extract machine name into PATH}
      string_downcase (path);          {we always list machine names in lower case}
      if not nodename_set then string_set_nodename; {make sure NODENAME is all set}
      if string_equal (path, nodename) then begin {this is our machine name ?}
...
comptype_rel_k: begin                  {component is relative to existing path}
  ...
  repeat
    ...
    t.str[t.len] := iname.str[st];
    st := st + 1;                      {advance source char index}
    len := len - 1;                    {one less character to copy}
    until len <= 0;                    {back and copy next component character}
  st := p;                             {unused path starts at start of next comp}
{
...
abort:
  if t.len >= t.max then return;       {punt if overflowed T}
  len := iname_len - st + 1;           {number of chars in remaining path}
  if (t.len > 0) and (st <= iname_len) then begin {need separator ?}
    t.len := t.len + 1;                {append "/" separator to T}
    t.str[t.len] := '/';
    end;
  while len > 0 do begin               {loop until remaining path exhausted}
    if t.len >= t.max then return;     {punt if overflowed T}
    t.len := t.len + 1;                {apppend this component name character to T}
